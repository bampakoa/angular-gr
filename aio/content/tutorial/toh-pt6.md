...
Παρατηρήστε ότι συνεχίζετε να εισάγετε το `MessageService` αλλά επειδή η εφαρμογή σας το καλεί τόσο συχνά, βάλτε το σε μια private μέθοδο `log()`:
...
Οι μεθόδοι του `HeroService` χρησιμοποιούνται στη ροή των τιμών του observable
...
Εδώ, το *base URL* είναι το `heroesURL` που ορίστηκε στην ενότητα [Ήρωες και HTTP](tutorial/toh-pt6#ήρωες-και-http) στο `api/heroes` και το *id* είναι ο αριθμός του ήρωα που θέλετε να ανακτήσετε. Για παράδειγμα, `api/heroes/11`.
...
Το `ng generate` δημιουργεί τα τρία αρχεία του `HeroSearchComponent` και προσθέτει το component στα declarations του `AppModule`.
